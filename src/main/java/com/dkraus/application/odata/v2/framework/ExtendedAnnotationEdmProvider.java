 ...
 * 
 */
public class ExtendedAnnotationEdmProvider extends EdmProvider {
	...
	public ExtendedAnnotationEdmProvider(final Collection<Class<?>> annotatedClasses) throws ODataException {
	...
	public ExtendedAnnotationEdmProvider(final String packageToScan) throws ODataException {
	...
	static class TypeBuilder {
		...
		private TypeBuilder withClass(final Class<?> aClass) {
			...
			for (Field field : fields) {
				...
				if (enp != null) {
					...
					AnnotationHelper.AnnotatedNavInfo info = ANNOTATION_HELPER.getCommonNavigationInfo(fromClass,
							toClass);
		...
		public EntityType buildEntityType() {
			...
			return entityType.setName(name).setAbstract(isAbstract).setHasStream(isMediaResource)
		...
		private Property createSimpleProperty(final EdmProperty ep, final Field field) {
			...
			sp.setAnnotationAttributes(addCustomAnnotationsToSimpleProperty(field));
			return sp;
		}

		private List<AnnotationAttribute> addCustomAnnotationsToSimpleProperty(Field field) {

			List<AnnotationAttribute> attributes = new ArrayList<>();

			final String CUSTOM_NAMESPACE = "sap";
			final String SAP_PREFIX = "sap";

			Sap sapAnnotation = field.getAnnotation(Sap.class);
			if (null != sapAnnotation) {

				if (!StringUtils.isEmpty(sapAnnotation.text())) {
					attributes.add(new AnnotationAttribute().setName("text").setNamespace(CUSTOM_NAMESPACE)
							.setPrefix(SAP_PREFIX).setText(sapAnnotation.text()));
				}

				if (!StringUtils.isEmpty(sapAnnotation.label())) {
					attributes.add(new AnnotationAttribute().setName("label").setNamespace(CUSTOM_NAMESPACE)
							.setPrefix(SAP_PREFIX).setText(sapAnnotation.text()));
				}

				if (!StringUtils.isEmpty(sapAnnotation.displayFormat())) {
					attributes.add(new AnnotationAttribute().setName("display-format").setNamespace(CUSTOM_NAMESPACE)
							.setPrefix(SAP_PREFIX).setText(sapAnnotation.displayFormat()));
				}

				attributes.add(new AnnotationAttribute().setName("creatable").setNamespace(CUSTOM_NAMESPACE)
						.setPrefix(SAP_PREFIX).setText(sapAnnotation.creatable() + ""));
				attributes.add(new AnnotationAttribute().setName("updatable").setNamespace(CUSTOM_NAMESPACE)
						.setPrefix(SAP_PREFIX).setText(sapAnnotation.updatable() + ""));
				attributes.add(new AnnotationAttribute().setName("sortable").setNamespace(CUSTOM_NAMESPACE)
						.setPrefix(SAP_PREFIX).setText(sapAnnotation.sortable() + ""));
				attributes.add(new AnnotationAttribute().setName("filterable").setNamespace(CUSTOM_NAMESPACE)
						.setPrefix(SAP_PREFIX).setText(sapAnnotation.filterable() + ""));
			}
			return attributes;
		}
